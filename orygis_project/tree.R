...
library("ape")
library("phytools")
setwd('~/gitprojects/mtbdiff')
anim <- read.table('ani_matrix.csv',sep=',',header=TRUE, row.names=1)
labels <- read.table('notebooks/orygis_samples.csv',sep=',',header=TRUE,row.names=1)
...
labels <- labels[rooted$tip.label,]
cols<-setNames(c('red','blue','white'),levels(as.factor(labels)))
